...
public class DeviceVerifyItemServerImpl implements DeviceVerifyItemServer{
	...
	//查询设备防错要求
	public PageInfo<DeviceVerifyItemVO> queryDeviceVerifyItem(Integer lineId, Integer page_size, Integer page_num, Integer shopId, Integer fcId) {
		if (page_num == null) {
			page_num = 1;
		}
		if (page_size == null) {
		...
		List<DeviceVerifyItemVO> deviceVerifyItems = deviceVerifyItemDao.queryDeviceVerifyItem(lineId, page_size, page_num, shopId, fcId);
